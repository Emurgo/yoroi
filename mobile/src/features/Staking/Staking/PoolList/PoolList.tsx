import {asQuantity, formatTokenWithText} from '@yoroi/cardano-wallet'
    ...
    return (
      <View style={[a.flex_1, a.justify_center, a.align_center, a.p_lg]}>
        <Text style={[a.body_1_lg_regular, ta.text_gray_medium]}>
          {strings.staking.failedToLoadPools}
        </Text>
      </View>
    )
  }
  ...
  const poolName = React.useMemo(
    () =>
      pool.name && pool.ticker
        ? `${pool.name} [${pool.ticker}]`
        : pool.name || pool.ticker || strings.dashboard.unknownPool,
    [pool.name, pool.ticker, strings.dashboard.unknownPool],
  )
      ...
      <View style={[a.flex_row, a.gap_md, a.align_center]}>
        ...
        <View style={[a.flex_1, a.gap_xs]}>
          ...
          <View style={[a.flex_row, a.gap_md, a.flex_wrap]}>
            {pool.roa && (
              <PoolStat
                label={strings.staking.poolROA}
                value={`${Number(pool.roa).toFixed(2)}%`}
              />
            )}

            {pool.saturation && (
              <PoolStat
                label={strings.staking.poolSaturation}
            ...
            {formattedStake && (
              <PoolStat
                label={strings.staking.poolStake}
                value={formattedStake}
              />
            )}
          </View>
        </View>
