...
async function discoverUsedAddressesByStakingCredential({
  ...
  for (const addressStr of allAddresses) {
    try {
      ...
      getLogger().warn('Failed to check address staking credential', {
        address: addressStr,
        error,
      })
    }
...
export const readOnlyAccountManagerMaker = async ({
  ...
  if (
    ...
    try {
      getLogger().debug('Discovering addresses by staking credential', {
        knownAddress,
      })
      ...
      getLogger().debug('Address discovery completed', {
        internalCount: finalInternalAddresses.length,
        externalCount: finalExternalAddresses.length,
      })
    } catch (error) {
      getLogger().error('Address discovery failed', {error})
    ...
    try {
      ...
      getLogger().warn('Failed to derive reward address', {
        error,
        knownAddress,
        chainId,
  ...
  const discoverAddresses = async (context?: {
    ...
    if (enableDiscovery && knownAddress) {
      ...
      if (!isValidCardanoAddress(knownAddress)) {
        getLogger().warn('Address re-discovery skipped: invalid address', {
      ...
      if (timeSinceLastDiscovery < DISCOVERY_THROTTLE_INTERVAL) {
        ...
        getLogger().debug('Address re-discovery skipped: throttled', {
      ...
      try {
        getLogger().debug('Starting address re-discovery', {
          knownAddress: knownAddress.substring(0, 20) + '...',
          chainId,
          timeSinceLastDiscovery,
        })
