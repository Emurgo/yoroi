...
const createTipStatusService = (
  ...
  const fetchTipStatus = async (
    ...
    try {
      ...
      getLogger().error('tipStatusService: Failed to fetch tip status', {
        network,
        error,
  ...
  const start = (): void => {
    ...
    fetchTipStatus().catch((error) => {
      getLogger().error('tipStatusService: Initial fetch error', {
        network,
        error,
        origin: 'TipStatusService',
      })
    ...
    refreshSubscription = interval(REFRESH_INTERVAL).subscribe(() => {
      fetchTipStatus().catch((error) => {
        getLogger().error('tipStatusService: Periodic refresh error', {
          network,
          error,
          origin: 'TipStatusService',
        })
