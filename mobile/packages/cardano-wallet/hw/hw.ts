...
const mapLedgerError = (e: unknown): Error | LocalizableError => {
  if (isAdaAppClosedError(e)) {
    ...
    getLogger().error('mapLedgerError: int64-buffer compatibility error', {e})
    return new Error(
    ...
    getLogger().error('mapLedgerError: Unexpected error', {e})
...
export const checkDeviceVersion = (
  ...
  if (minVersionArray.length !== deviceVersionArray.length) {
    getLogger().warn('checkDeviceVersion: version formats mismatch', {
      minVersionArray,
      deviceVersionArray,
    })
    return
...
const connectionHandler = async (
  ...
  try {
    ...
    getLogger().debug('connectionHandler: AppAda version', {versionResp})
    checkDeviceVersion(versionResp)

    return appAda
  } catch (e) {
    getLogger().error('connectionHandler error', {error: e, useUSB, deviceId})
...
export const getHWDeviceInfo = async (
  ...
  try {
    ...
    getLogger().error('getHWDeviceInfo error', {
      error: e,
      implementation,
      deviceId,
      deviceObj,
...
export const getCardanoAppMajorVersion = async (
  ...
  getLogger().debug('getCardanoAppMajorVersion: version', {version})
