...
import {YoroiWallet, getTipStatusService} from '@yoroi/cardano-wallet'
...
export const makeSyncManager = (
  ...
  const performSync = async (isForced: boolean = false) => {
    ...
    if (!tipStatus) {
      getLogger().warn('syncManager: No tip status available, skipping sync', {
  ...
  const start = () => {
      ...
      combineLatest([interval(config.normalInterval), syncState$]).pipe(
        switchMap(([_, state]) => {
          ...
          return timer(0, currentInterval).pipe(
            ...
            catchError((error) => {
              getLogger().error('syncManager: Polling sync error', {error})
              return of(null)
            }),
          )
        }),
      ...
      txSubmitted$.pipe(
        switchMap((event) => {
          ...
          return from(
            ...
            catchError((error) => {
              getLogger().error('syncManager: Transaction sync error', {error})
              return of(null)
            }),
          )
        }),
    ...
    performSync(false).catch((error) => {
      getLogger().error('syncManager: Initial sync error', {error})
    })
  }
  ...
  const triggerSync = async (walletId: YoroiWallet['id']) => {
    ...
    syncWalletsParallel([wallet], config, syncState$.value, true, tipStatus)
      ...
      .catch((error) => {
        getLogger().error('syncManager: Trigger sync error', {walletId, error})
      })
  }
