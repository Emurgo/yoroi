...
export class BatchedEmurgoUtxoApi implements UtxoApiContract {
  ...
  constructor(base: UtxoApiContract, maxAddresses = 500) {
    this._base = base
...
export class EmurgoUtxoApi implements UtxoApiContract {
  ...
  private throwRequestErrors: boolean
  private pageSize: number

  constructor(apiUrl: string, throwRequestErrors: boolean, pageSize = 50) {
    this.apiUrl = apiUrl
    this.throwRequestErrors = throwRequestErrors
  ...
  async getUtxoAtPoint(
    ...
    try {
      ...
      while (utxosAtPointPage.length === this.pageSize) {
        page++
