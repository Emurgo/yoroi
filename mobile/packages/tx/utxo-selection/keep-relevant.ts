...
/**
 * Keep Relevant coin selection algorithm (CIP-2 compliant)
 *
 * First selects UTXOs that contain required assets (to minimize change),
 * then uses largest first for remaining ADA requirements.
 *
...
export function keepRelevant(
  ...
  // Step 1: Select UTXOs that contain required assets
  if (requiredTokenIds.length > 0) {
    for (const utxo of availableUtxos) {
      ...
      if (utxoHasRelevantAssets(utxo, requiredAmounts)) {
        selected.push(utxo)
  ...
  // Step 2: Check if we still need more (especially ADA)
  if (!hasEnoughAmounts(selectedAmounts, requiredAmounts)) {
    ...
    // Select additional UTXOs until we have enough
