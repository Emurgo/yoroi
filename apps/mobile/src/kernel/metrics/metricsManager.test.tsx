...
describe('makeMetricsManager', () => {
  ...
  test('track should call the appropriate metricsModule methods', () => {
    ...
    metricsManager.track.nftGallerySearchActivated({
      nft_search_term: 'test',
      nft_count: 10,
    })
    ...
    metricsManager.track.swapInitiated({
      from_asset: [
        {asset_name: 'ADA', asset_ticker: 'ADA', policy_id: '12345'},
      ],
      to_asset: [
        {asset_name: 'DOGE', asset_ticker: 'DOGE', policy_id: '65432'},
      ],
      order_type: 'limit',
    ...
    metricsManager.track.governanceConfirmTransactionPageViewed({
      governance_selection: 'Delegate',
    })
    metricsManager.track.governanceConfirmTransactionPageViewed({
      governance_selection: 'Abstain',
    })
    metricsManager.track.governanceConfirmTransactionPageViewed({
      governance_selection: 'No Confidence',
    })
    metricsManager.track.governanceDashboardPageViewed()
    metricsManager.track.governanceTransactionSuccessPageViewed({
      governance_selection: 'Delegate',
    })
    metricsManager.track.governanceTransactionSuccessPageViewed({
      governance_selection: 'Abstain',
    ...
    metricsManager.track.restoreWalletEnterPhraseStepStatus({
      recovery_prhase_status: true,
    })
    metricsManager.track.restoreWalletEnterPhraseStepStatus({
      recovery_prhase_status: false,
    })
    metricsManager.track.restoreWalletEnterPhraseStepViewed({
    ...
    metricsManager.track.receiveCopyAddressClicked({
      copy_address_location: 'CTA Copy Address',
    })
    metricsManager.track.receiveCopyAddressClicked({
    ...
    metricsManager.track.portfolioTokenDetails({
      token_details_tab: 'Performance',
    })
    metricsManager.track.portfolioTokenDetails({
      token_details_tab: 'Transactions',
    })
    metricsManager.track.portfolioTokenDetails({token_details_tab: 'Overview'})
    metricsManager.track.portfolioTokensListPageViewed({
      tokens_tab: 'Wallet Token',
    })
    metricsManager.track.portfolioTokensListPageViewed({
      tokens_tab: 'Dapps Token',
    })
    metricsManager.track.portfolioTokensListSearchActivated({
      search_term: 'token',
    })
    ...
    metricsManager.track.discoverWebViewToolbarSearchActivated({
      search_term: 'random',
    })
    metricsManager.track.discoverWebViewViewed()

    metricsManager.track.networkSelected({
      to_network: 'preprod',
      from_network: 'preview',
    })
    ...
    metricsManager.track.settingsInAppNotificationsStatusUpdated({
      status: 'disabled',
    })
    ...
    metricsManager.track.settingsPushNotificationsStatusUpdated({
      is_enabled: 'enabled',
    })
    ...
    expect(mockAmpli.nftGalleryDetailsTab).toHaveBeenCalledWith({
      nft_tab: 'Metadata',
    })
    expect(mockAmpli.nftGalleryPageViewed).toHaveBeenCalledWith({nft_count: 10})
    expect(mockAmpli.nftGallerySearchActivated).toHaveBeenCalledWith({
      nft_search_term: 'test',
      nft_count: 10,
    })
    ...
    expect(mockAmpli.sendSelectAssetSelected).toHaveBeenCalledWith(
      mockSendPayload,
    )
    expect(mockAmpli.sendSelectAssetUpdated).toHaveBeenCalledWith(
      mockSendPayload,
    )
    expect(mockAmpli.sendSummaryPageViewed).toHaveBeenCalledWith(
      mockSendPayload,
    )
    ...
    expect(mockAmpli.exchangeSubmitted).toHaveBeenCalledWith({
      ramp_type: 'Buy',
      ada_amount: 222,
    })
    ...
    expect(
      mockAmpli.governanceConfirmTransactionPageViewed,
    ).toHaveBeenCalledWith({governance_selection: 'Delegate'})
    expect(
      mockAmpli.governanceConfirmTransactionPageViewed,
    ).toHaveBeenCalledWith({governance_selection: 'Abstain'})
    expect(
      mockAmpli.governanceConfirmTransactionPageViewed,
    ...
    expect(
      mockAmpli.governanceTransactionSuccessPageViewed,
    ).toHaveBeenCalledWith({governance_selection: 'Delegate'})
    expect(
      mockAmpli.governanceTransactionSuccessPageViewed,
    ).toHaveBeenCalledWith({governance_selection: 'Abstain'})
    ...
    expect(mockAmpli.restoreWalletEnterPhraseStepStatus).toHaveBeenCalledWith({
      recovery_prhase_status: true,
    })
    expect(mockAmpli.restoreWalletEnterPhraseStepStatus).toHaveBeenCalledWith({
      recovery_prhase_status: false,
    })
    expect(mockAmpli.restoreWalletEnterPhraseStepViewed).toHaveBeenCalledWith({
      recovery_phrase_lenght: '15',
    })
    expect(mockAmpli.restoreWalletEnterPhraseStepViewed).toHaveBeenCalledWith({
      recovery_phrase_lenght: '24',
    ...
    expect(mockAmpli.receiveAmountGeneratedPageViewed).toHaveBeenCalledWith({
      ada_amount: 500,
    })
    ...
    expect(mockAmpli.portfolioTokenDetails).toHaveBeenCalledWith({
      token_details_tab: 'Performance',
    })
    expect(mockAmpli.portfolioTokenDetails).toHaveBeenCalledWith({
      token_details_tab: 'Transactions',
    ...
    expect(mockAmpli.portfolioTokenDetails).toHaveBeenCalledWith({
      token_details_tab: 'Overview',
    })
    expect(mockAmpli.portfolioTokensListPageViewed).toHaveBeenCalledWith({
      tokens_tab: 'Wallet Token',
    })
    expect(mockAmpli.portfolioTokensListPageViewed).toHaveBeenCalledWith({
      tokens_tab: 'Dapps Token',
    })
    expect(mockAmpli.portfolioTokensListSearchActivated).toHaveBeenCalledWith({
      search_term: 'token',
    })

    expect(
      mockAmpli.discoverConnectedBottomSheetDisconnectClicked,
    ).toHaveBeenCalled()
    expect(
      mockAmpli.discoverConnectedBottomSheetOpenDAppClicked,
    ).toHaveBeenCalled()
    ...
    expect(mockAmpli.discoverFilterSelected).toHaveBeenCalledWith({
      dapp_filter: 'Investment',
    })
    expect(mockAmpli.discoverFilterSelected).toHaveBeenCalledWith({
      dapp_filter: 'Media',
    })
    ...
    expect(mockAmpli.discoverFilterSelected).toHaveBeenCalledWith({
      dapp_filter: 'Community',
    })
    expect(mockAmpli.discoverPageViewed).toHaveBeenCalled()
    expect(mockAmpli.discoverSearchActivated).toHaveBeenCalledWith({
      search_term: 'random',
    })
    expect(
      mockAmpli.discoverWebViewBottomSheetConnectClicked,
    ).toHaveBeenCalled()
    ...
    expect(
      mockAmpli.discoverWebViewToolbarSearchActivated,
    ).toHaveBeenCalledWith({search_term: 'random'})
    expect(mockAmpli.discoverWebViewViewed).toHaveBeenCalled()

    expect(mockAmpli.networkSelected).toHaveBeenCalledWith({
      to_network: 'preprod',
      from_network: 'preview',
    })
    ...
    expect(mockAmpli.inAppNotificationOpened).toHaveBeenCalledWith({
      type: 'tx_received',
    })
    expect(mockAmpli.inAppNotificationClosed).toHaveBeenCalledWith({
      type: 'tx_received',
    })
    expect(mockAmpli.inAppNotificationViewed).toHaveBeenCalled()
    expect(
      mockAmpli.settingsInAppNotificationsStatusUpdated,
    ).toHaveBeenCalledWith({status: 'disabled'})
