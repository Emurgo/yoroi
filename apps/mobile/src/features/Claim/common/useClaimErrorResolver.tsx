...
export const useClaimErrorResolver = () => {
  ...
  const resolver = (error: unknown) => {
    if (error instanceof Claim.Api.Errors.AlreadyClaimed)
      return dialogs.errorAlreadyClaimed
    if (error instanceof Claim.Api.Errors.Expired) return dialogs.errorExpired
    if (error instanceof Claim.Api.Errors.InvalidRequest)
      return dialogs.errorInvalidRequest
    if (error instanceof Claim.Api.Errors.NotFound) return dialogs.errorNotFound
    if (error instanceof Claim.Api.Errors.RateLimited)
      return dialogs.errorRateLimited
