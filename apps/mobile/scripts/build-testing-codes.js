...
const generateQRCode = (content, filename) => {
  const cmd = `qrencode -o "${filename}_qr.png" "${shellEscape(content)}"`
  console.log(cmd)
  execSync(cmd)
}

const generatePDF417Code= (content, filename) => {
  const cmd = `zint -b 55 -o "${filename}_pdf417.png" -d "${shellEscape(content)}"`
  console.log(cmd)
  execSync(cmd)
}
...
const processObject = (obj, prefix = '') => {
  for (const key in obj) {
    ...
    const value = obj[key]
    const filename = path.join(__dirname, '../test-codes', `${prefix}${key}`)
    if (typeof value === 'string') {
      generateQRCode(value, filename)
      generatePDF417Code(value, filename);
