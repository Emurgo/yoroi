...
import {tokenManagerMaker} from './tokenManager' // Replace 'your-module' with the actual module path

describe('tokenManagerMaker', () => {
  it('should refresh tokens without avoidCache', async () => {
    ...
    const tokenManagerToInit = tokenManagerMaker({storage: mockStorage, api: mockApi})
    const tokenManager = await tokenManagerToInit()

    // Act
    await tokenManager.update(['token1', 'token2'])
  ...
  it('should refresh tokens with avoidCache', async () => {
    ...
    // Act
    await tokenManager.update(['token1', 'token2'], true)
