...
import {getOffChainMetadata, isTokenRegistryEntry} from './token-offchain-metadata'
...
describe('getOffChainMetadata', () => {
  ...
  it('should fetch metadata for multiple tokens', async () => {
    const mockFetch = jest.fn<Promise<ApiTokenRegistryEntry>, any[]>(({endpoint}) => {
      if (endpoint.endsWith('token1')) {
        return Promise.resolve({
          subject: 'token1',
          name: {signatures: [], sequenceNumber: 1, value: 'Token1'},
        })
      } else if (endpoint.endsWith('token2')) {
        return Promise.resolve({
          subject: 'token2',
          name: {signatures: [], sequenceNumber: 1, value: 'Token2'},
        })
      } else if (endpoint.endsWith('token3')) {
    ...
    const fetchMetadata = getOffChainMetadata(baseUrl, mockFetch)
    const result = await fetchMetadata(['token.1', 'token.2', 'token.3', 'token.4'])

    expect(result).toEqual({
      'token.1': {
        tokenRegistry: {subject: 'token1', name: {signatures: [], sequenceNumber: 1, value: 'Token1'}},
        isValid: true,
      },
      'token.2': {
        tokenRegistry: {subject: 'token2', name: {signatures: [], sequenceNumber: 1, value: 'Token2'}},
        isValid: true,
      },
  ...
  it('should handle for failed fetches', async () => {
    const mockFetch = jest.fn<Promise<ApiTokenRegistryEntry>, any[]>(() => {
    ...
    expect(result).toEqual({'token.1': {tokenRegistry: undefined, isValid: false}})
  })
