import {isFtMetadata, isMetadataFile, isNftMetadata, parseFtMetadataRecord, parseNftMetadataRecord} from './parsers' // Replace with the actual module path
...
describe('isMetadataFile', () => {
  ...
  it('should return false if name exists but is not a string', () => {
    expect(isMetadataFile({name: 123, mediaType: 'image/png', src: 'some-src'})).toBe(false)
  })
  ...
  it('should return false if src is not a string or array of strings', () => {
    ...
    expect(isMetadataFile({mediaType: 'image/png', src: [123]})).toBe(false)
    expect(isMetadataFile({mediaType: 'image/png', src: ['valid', 123]})).toBe(false)
  })
  ...
  it('should return true if all fields are valid', () => {
    expect(isMetadataFile({name: 'some-name', mediaType: 'image/png', src: 'some-src'})).toBe(true)
    expect(isMetadataFile({name: 'some-name', mediaType: 'image/png', src: ['some-src']})).toBe(true)
  })
...
describe('isNftMetadata', () => {
  ...
  it('should return false if files exists but is not an array of valid MetadataFile', () => {
    expect(isNftMetadata({name: 'some-name', files: [{name: 123, mediaType: 'image/png', src: 'some-src'}]})).toBe(
      false,
    )
    expect(isNftMetadata({name: 'some-name', files: [{name: 123, mediaType: 1, src: 'some-src'}]})).toBe(false)
    expect(isNftMetadata({name: 'some-name', mediaType: 1, image: ''})).toBe(false)
  })

  it('should return true if only the required fields are valid and others are not present', () => {
    expect(isNftMetadata({name: 'some-name', image: 'data:image/png:some-image'})).toBe(true)
  })

  it('should return false if description type is invalid', () => {
    expect(isNftMetadata({name: 'some-name', image: 'data:image/png:some-image', description: 1})).toBe(false)
  })
  ...
  it('should return true if all fields are valid and have or not extra fields', () => {
    expect(
      isNftMetadata({
        ...
        image: 'some-image',
        files: [{name: 'some-file-name', mediaType: 'image/png', src: 'some-src'}],
        extra: 1,
      }),
    ).toBe(true)
    expect(
