import {isArrayOfType, isPositiveNumber, isRecord, isString, isStringOrArrayOfString} from '@yoroi/wallets'
...
export function isNftMetadata(data: unknown): data is ApiNftMetadata {
  ...
  if (metadata.description && !isStringOrArrayOfString(metadata.description)) return false

  if (metadata.files && !isArrayOfType(metadata.files, isMetadataFile) && !isMetadataFile(metadata.files)) return false
...
export function isFtMetadata(data: unknown): data is ApiFtMetadata {
  ...
  if (metadata.decimals !== undefined && !isPositiveNumber(metadata.decimals)) return false

  return true
}

export function parseNftMetadataRecord(data: Readonly<Record<string, unknown>>) {
