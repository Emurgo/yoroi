...
import {decimalsResolver, nameResolver} from './helpers/tokens'
  ...
  const updatePortfolio = async (utxos: ReadonlyArray<RawUtxo>, initialPrimaryToken: T) => {
    const task = async () => {
      ...
      const primaryBalance: Portfolio.TokenBalance<T> = {
        ...initialPrimaryToken,
        balance: {
          quantity: primaryAmount[primaryTokenId],
          isPrimary: true,
          resolvedName: nameResolver(initialPrimaryToken.info),
          resolvedDecimals: decimalsResolver(initialPrimaryToken.info),
        },
      }
      ...
      const primaryLockedBalance: Portfolio.TokenBalance<T> = {
        ...initialPrimaryToken,
        balance: {
          quantity: await calcLockedDeposit(utxos),
          isPrimary: true,
          resolvedName: nameResolver(initialPrimaryToken.info),
          resolvedDecimals: decimalsResolver(initialPrimaryToken.info),
        },
      ...
      secondaryIds.forEach((tokenId) => {
        ...
        const secondaryBalance = {
          ...token,
          balance: {
            quantity: secondaryAmounts[tokenId],
            isPrimary: false,
            resolvedName: nameResolver(token.info),
            resolvedDecimals: decimalsResolver(token.info),
          },
        } as const
