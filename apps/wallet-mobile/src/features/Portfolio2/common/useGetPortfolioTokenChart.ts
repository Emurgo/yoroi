...
function generateMockChartData(
    ...
    for (let i = 0; i < dataPoints; i++) {
        ...
        const label = `${date.getDate().toString().padStart(2, '0')}/${(date.getMonth() + 1).toString().padStart(2, '0')}/${date.getFullYear().toString().substr(-2)} ${date.getHours()}:${date.getMinutes().toString().padStart(2, '0')}`;
...
const useGetPortfolioTokenChart = (
    ...
    options: UseQueryOptions<ITokenChartData[], Error, ITokenChartData[], ['useGetPortfolioTokenChart', TokenChartTimeInterval]> = {},
) => {
