...
for pkg in "${PACKAGES[@]}"; do
  ...
  mobile_pkg_dir="apps/mobile/packages/$pkg"
  
  if [ -d "$scripts_pkg_dir" ] && [ -d "$mobile_pkg_dir" ]; then
    # Create relative symlink from scripts/packages/<pkg>/src to apps/mobile/packages/<pkg>
    cd "$scripts_pkg_dir"
    ln -sf "../../../apps/mobile/packages/$pkg" "src"
    cd - > /dev/null
