...
export const termsOfService = [
  paragraph(
    'ЭТО СОГЛАШЕНИЕ О ПРЕДОСТАВЛЕНИИ УСЛУГ (далее "Соглашение") заключаются между EMURGO \
Co., Ltd. (далее "Компания") и любым физическим или юридическим лицом ( далее - “Пользователь”), который завершает \
процесс загрузки, утилизации или эксплуатации программного обеспечения, известного как приложение \
криптовалютного кошелька Yoroi, а также услуги по обработке данных, приложения, \
коммуникационных услуг и другого контента, предлагаемого или предоставляемого \
программным обеспечением Компании (далее “Программа”). Компания и Пользователь, совместно \
далее именуются “Сторонами”. ПОДТВЕРДИВ СОГЛАСИЕ, НАЖАТИЕМ КНОПКИ ИЛИ ПОЛУЧЕНИЕМ ДОСТУПА, \
ИСПОЛЬЗУЯ ИЛИ УСТАНАВЛИВАЯ КАКУЮ-ЛИБО ЧАСТЬ ПРОГРАММНОГО ОБЕСПЕЧЕНИЯ, ПОЛЬЗОВАТЕЛЬ НЕПОСРЕДСТВЕННО ПРИНИМАЕТ УСЛОВИЯ СОГЛАШЕНИЯ И \
ДАЕТ СОГЛАСИЕ БЫТЬ ЮРИДИЧЕСКИ СВЯЗАННЫМ СО ВСЕМИ СОГЛАШЕНИЯМИ ДАННОГО ДОГОВОРА. ЕСЛИ ПОЛЬЗОВАТЕЛЬ \
НЕ ДАЕТ СВОЕ СОГЛАСИЕ НА ВСЕ УСЛОВИЯ ЭТОГО ДОГОВОРА, ПОЛЬЗОВАТЕЛЮ НЕ БУДЕТ \
РАЗРЕШЕН ДОСТУП, ИСПОЛЬЗОВАНИЕ ИЛИ УСТАНОВКА КАКОЙ-ЛИБО ЧАСТИ ПРОГРАММЫ.',
  ),
