...
import {signTxWithLedger} from '../../../shared/hw'
import {
  ACCOUNT_INDEX,
  API_ROOT,
  BACKEND,
...
import {NetworkInfo, networkInfo} from '../../../mainnet/shelley/networkInfo'
...
export class ShelleyWallet implements YoroiWallet {
  ...
  static async create({
    ...
    const wallet: any = await this.commonCreate({
      id,
      storage,
      accountPubKeyHex,
  ...
  private static commonCreate = async ({
    ...
    const rewardAddressHex = await deriveRewardAddressHex(accountPubKeyHex)
