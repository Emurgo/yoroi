...
import { ShelleyWallet } from '../shelley-15'
...
describe('migration', () => {
  ...
  it('create', async () => {
    ...
    expect(wallet.internalAddresses).toMatchSnapshot('create, wallet.internalAddresses')
    expect(wallet.externalAddresses).toMatchSnapshot('create, wallet.externalAddresses')
    ...
    await expect(getWalletData(wallet)).resolves.toMatchSnapshot('wallet data')

    expect(wallet.networkInfo.getTime(1676911252744)).toEqual({
      epoch: 395,
      slot: 68161,
      absoluteSlot: 170708161,
      percentageElapsed: 15,
      secondsRemainingInEpoch: 363839,
      slotsPerEpoch: 432000,
      slotsRemaining: 363839,
    })
  })
  ...
  it('restore', async () => {
    ...
    const wallet: Record<string, any> = await ShelleyWallet.restore({
      storage: storage.join(`${walletMeta.id}/`),
      walletMeta,
    })
    ...
    expect(wallet.utxos).toMatchInlineSnapshot(`Array []`)
    expect(wallet.internalAddresses).toMatchSnapshot('restore: wallet.internalAddresses')
    expect(wallet.externalAddresses).toMatchSnapshot('restore: wallet.externalAddresses')
    expect(wallet.transactions).toMatchInlineSnapshot(`Object {}`)
    expect(wallet.state).toMatchInlineSnapshot(`
      Object {
  ...
  it('create hw ', async () => {
    ...
    expect(wallet.utxos).toMatchInlineSnapshot(`Array []`)
    expect(wallet.internalAddresses).toMatchSnapshot('create hw: wallet.internalAddresses')
    expect(wallet.externalAddresses).toMatchSnapshot('create hw: wallet.externalAddresses')
    expect(wallet.transactions).toMatchInlineSnapshot(`Object {}`)
    expect(wallet.state).toMatchInlineSnapshot(`
          Object {
            "lastGeneratedAddressIndex": 0,
    ...
    await expect(getWalletData(wallet)).resolves.toMatchSnapshot('create hw: wallet data')
...
const getWalletData = (wallet: any) => storage.join(`${wallet.id}/`).getItem(`data`)
