...
<head>
  ...
  <script>
    ...
    function generate(_seed, _size, _scale, _bgColor, _color, _spotColor) {
      var icon = document.getElementById('icon');
      icon.style.width = '130px'
      icon.style.height = '130px'
      icon.style.backgroundImage = 'url(' + blockies.create({seed: _seed, size: _size, scale: _scale, bgColor: _bgColor, color: _color, spotColor: _spotColor}).toDataURL()+')'
    }
  </script>
</head>
<body>
    ...
    return (
      <View style={[{backgroundColor: 'transparent'}, this.props.style]}>
        <WebView
          ...
          scrollEnabled={true}
          injectedJavaScript={`generate(
            '${this.props.blockies}',
            ${this.props.size},
            ${this.props.scale},
            '${this.props.bgColor}',
            '${this.props.color}',
            '${this.props.spotColor}',
          )`}
        />
      </View>
    )
