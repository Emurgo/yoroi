...
export const onDidMount = <Props, Callback: (any) => mixed>(
  didMount: Callback,
): HOC<Props, Props> => (
...
export const onWillUnmount = <Props, Callback: (any) => mixed>(
...
export const withNavigationTitle = <Props: {navigation: any}>(
  ...
  class WithScreenTitle extends React.Component<Props> {
    ...
    getCurrentTitle = () => this.props.navigation.getParam(paramName || 'title')
...
// Note that simply replacing 'any' with Props (or Subprops)
// turns flow into I-ignore-type-errors beast :-(
export const requireLoaded = <Props, Callback: (any) => mixed>(
  isLoaded: Callback,
  Loading: ComponentType<{}> = () => null,
): HOC<Props, Props> => (
  BaseComponent: ComponentType<Props>,
): ComponentType<Props> => (props) =>
  isLoaded(props) ? <BaseComponent {...props} /> : <Loading />
