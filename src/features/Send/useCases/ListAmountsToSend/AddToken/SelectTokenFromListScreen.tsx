...
const ListEmptyComponent = ({
  ...
  const amountsSelected = Object.keys(targets[selectedTargetIndex].entry.amounts)
  const allAssetsSelected = getAllAssetsSelected(amountsSelected, allTokenInfos)
  const strings = useStrings()
  ...
  if ((isWalletEmpty || allAssetsSelected) && !isSearching)
...
const getAllAssetsSelected = (amountsSelected: Array<string>, tokenInfos): boolean =>
  tokenInfos.every((tokenInfo) => amountsSelected.includes(tokenInfo.id))
const filterOutSelected = (amountsSelected: Array<string>) => (token: YoroiNft | TokenInfo) =>
  !amountsSelected.includes(token.id)
