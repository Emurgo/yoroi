...
export const generateVotingKeys = () => async (dispatch: Dispatch<any>, _getState: () => State) => {
  ...
  const catalystEncryptedPrivateKey = await encryptWithPassword(passBuff, await (await rootKey.to_raw_key()).as_bytes())

  await dispatch(
    _setCatalystKeys({
      pin: pinArray,
      encryptedKey: catalystEncryptedPrivateKey,
      catalystPrivateKey: Buffer.from(await (await rootKey.to_raw_key()).as_bytes()).toString('hex'),
    }),
  )
...
export const generateVotingTransaction =
  (decryptedKey: string | void) => async (dispatch: Dispatch<any>, getState: () => State) => {
    ...
    if (utxos == null) {
      try {
        ...
        Logger.debug('voting actions::generateVotingTransaction: could not get utxos')
        dispatch(_setUnsignedTx(null))
      }
    }
    utxos = utxosSelector(getState())
    if (utxos == null) {
      throw new Error('voting actions::generateVotingTransaction: failed to fetch utxos')
    }

    if (catalystPrivateKey) {
      const signRequest = await walletManager.createVotingRegTx(utxos, catalystPrivateKey, decryptedKey, serverTime)
