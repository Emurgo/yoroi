...
describe('Create unsigned TX from UTXO', () => {
  ...
  it('Should pick ada-only inputs when using input selection', async () => {
    ...
    expect(
      await (
        await (await unsignedTxResponse.txBuilder.get_explicit_input()).coin()
      ).to_str(),
    ).toEqual('1000702')
    expect(
      await (
        await (await unsignedTxResponse.txBuilder.get_explicit_output()).coin()
      ).to_str(),
    ).toEqual('999536')
    expect(
  ...
  it('Should pick inputs with tokens when using input selection', async () => {
    ...
    // expect(await (await ((await (await unsignedTxResponse.txBuilder.get_explicit_input()).multiasset())
    //   ?.get(assetInfo.policyId))
    //   ?.get(assetInfo.name))
    //   ?.to_str()
    // ).toEqual('1234')
    let _multiAsset = await (
    ...
    _multiAsset = await (
      await (await (await tx.outputs()).get(1)).amount()
    ).multiasset()
    // prettier-ignore
    _assetAmountStr =
      _multiAsset != null
...
describe('Create signed transactions', () => {
  ...
  it('Transaction should support certificates', async () => {
    ...
    const stakingKey = await (
      await (
        await accountPrivateKey.derive(2)
          ...
          await (
            await make_vkey_witness(
              await hash_transaction(
                await unsignedTxResponse.txBuilder.build(),
              ),
              stakingKey,
            )
          ).to_bytes(),
  ...
  it('Transaction should support withdrawals', async () => {
    ...
    expect(
      await (
        await (await (await (await txBody.outputs()).get(0)).amount()).coin()
      ).to_str(),
    ).toEqual(
...
describe('Create sendAll unsigned TX from UTXO', () => {
  it('Create a transaction involving all input with no change', async () => {
    ...
    expect(
      await (
        await (await sendAllResponse.txBuilder.get_explicit_input()).coin()
      ).to_str(),
    ).toEqual(expectedInput.toString())
    expect(
      await (
        await (await sendAllResponse.txBuilder.get_explicit_output()).coin()
      ).to_str(),
