import { z } from 'zod';
import { createTypeGuardFromSchema, isArray, isArrayOfType, isBoolean, isNonNullable, isNumber, isRecord, isString, parseBoolean, parseSafe, parseString } from './parsers';
describe('parsers', () => {
  ...
  describe('parseSafe', () => {
    it('should return parsed JSON if valid JSON', () => {
      expect(parseSafe('{"key": "value"}')).toEqual({
        key: 'value'
      });
    });
  ...
  describe('createTypeGuardFromSchema', () => {
    const isPerson = createTypeGuardFromSchema(z.object({
      name: z.string(),
      age: z.number()
    }));
