...
export const initCatalyst = (cardano: CardanoTypes.Wasm) => {
  const generateWalletRootKey = async (mnemonic: string) => {
    const bip39entropy = mnemonicToEntropy(mnemonic)
    const EMPTY_PASSWORD = Buffer.from('')
    return await cardano.Bip32PrivateKey.fromBip39Entropy(
      Buffer.from(bip39entropy, 'hex'),
      EMPTY_PASSWORD,
    )
  }

  const auxiliaryDataWithRegistrationMetadata = async (request: {
    ...
    const registrationData = await cardano.encodeJsonStrToMetadatum(jsonMeta, 1)
    const metadata = await cardano.GeneralTransactionMetadata.new()
    await metadata.insert(
      await cardano.BigNum.fromStr(CatalystLabels.DATA.toString()),
      registrationData,
    )
    ...
    await metadata.insert(
      await cardano.BigNum.fromStr(CatalystLabels.SIG.toString()),
      await cardano.encodeJsonStrToMetadatum(
        JSON.stringify({
    ...
    const metadataList = await cardano.MetadataList.new()
    await metadataList.add(
      await cardano.TransactionMetadatum.fromBytes(await metadata.toBytes()),
    )
    await metadataList.add(
      await cardano.TransactionMetadatum.newList(
        await cardano.MetadataList.new(),
      ),
    )
    return await cardano.AuxiliaryData.fromBytes(await metadataList.toBytes())
  }

  async function generatePrivateKeyForCatalyst() {
  ...
  return {
    ...
    auxiliaryDataWithRegistrationMetadata,
