import {darkPalette} from '../base-palettes/dark-palette'
import {tokens} from '../tokens/tokens'
import {ThemedPalette} from '../types'

const {opacity} = tokens

export const black: ThemedPalette = {
  ...darkPalette,

  text_primary_max: darkPalette.primary_c700, // hover, text, button, links, text in tabs, chips
  text_primary_medium: darkPalette.primary_c600, // links, tabs, chips, special cases
  text_primary_min: darkPalette.primary_c300, // disabled, buttons, links
  text_gray_max: darkPalette.gray_cmax, // hover and pressed
  text_gray_medium: darkPalette.gray_c900, // accent or additional text
  text_gray_low: darkPalette.gray_c600, // disabled text
  text_gray_min: darkPalette.gray_c400, // disabled text
  text_error: darkPalette.sys_magenta_c500, // error messages
  text_warning: darkPalette.sys_orange_c500, // warning messages
  text_success: darkPalette.secondary_c500, // success messages
  text_info: darkPalette.sys_cyan_c500, // info messages

  bg_color_max: darkPalette.gray_c50, // bottom surface
  bg_color_min: darkPalette.gray_c100, // upper surface

  el_primary_max: darkPalette.primary_c700, // hover'nd pressed state, actianable elements
  el_primary_medium: darkPalette.primary_c600, // actionable elements
  el_primary_min: darkPalette.primary_c300, // disabled elements, icons
  el_gray_max: darkPalette.gray_cmax, // hover and pressed, icons, shapes, lines in buttons, icons in banners
  el_gray_medium: darkPalette.gray_c900, // icons, shapes, inputs, bottom navigation bar
  el_gray_low: darkPalette.gray_c600, // input stroke, disabled state for most components
  el_gray_min: darkPalette.gray_c400, // input stroke, disabled state for most components
  el_secondary: darkPalette.secondary_c600, // success state

  mobile_overlay: `${darkPalette.black_static}${opacity._64}`, // mobile bottom sheet overlay
  mobile_bg_blur: `${darkPalette.gray_c50}${opacity._80}`, // mobile bottom sheet background

  web_overlay: `${darkPalette.gray_c100}${opacity._80}`, // extension modal overlay
  web_bg_sidebar_active: `${darkPalette.black_static}${opacity._16}`, // extension active sidebar item background
  web_bg_sidebar_inactive: `${darkPalette.white_static}${opacity._48}`, // extension inactive sidebar item
} as const
