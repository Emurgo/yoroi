...
describe('dappConnectorApiMaker', () => {
  describe('getDApps', () => {
    it('should throw error if fetching fails', async () => {
      ...
      await expect(() => dappConnectorApiMaker({request: fakeFetchData}).getDApps({networkId: 1})).rejects.toThrow()
    })
    ...
    it('should throw error if response is invalid', async () => {
      ...
      await expect(() =>
        dappConnectorApiMaker({request: fakeFetchData as any}).getDApps({networkId: 1}),
      ).rejects.toThrow()
    })
    ...
    it('should return data if response is valid', async () => {
      ...
      const fakeFetchData = () => Promise.resolve(fakeResult)
      const result = await dappConnectorApiMaker({request: fakeFetchData as any}).getDApps({networkId: 1})
    ...
    it('should support preprod', async () => {
      const fakeResult = {tag: 'right' as const, value: {data: await managerMock.getDAppList()}} as const
      const fakeFetchData = () => Promise.resolve(fakeResult)
      const result = await dappConnectorApiMaker({request: fakeFetchData as any}).getDApps({networkId: 0})
      expect(result).toEqual({
        ...fakeResult.value.data,
        dapps: fakeResult.value.data.dapps.map((d) => ({...d, logo: 'https://daehx1qv45z7c.cloudfront.net/icon.png'})),
      })
    ...
    it('should support sancho', async () => {
      const fakeResult = {tag: 'right' as const, value: {data: await managerMock.getDAppList()}} as const
      const fakeFetchData = () => Promise.resolve(fakeResult)
      const result = await dappConnectorApiMaker({request: fakeFetchData as any}).getDApps({networkId: 450})
      expect(result).toEqual({
        ...fakeResult.value.data,
        dapps: fakeResult.value.data.dapps.map((d) => ({...d, logo: 'https://daehx1qv45z7c.cloudfront.net/icon.png'})),
      })
