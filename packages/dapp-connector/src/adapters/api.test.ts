...
describe('dappConnectorApiMaker', () => {
  describe('getDApps', () => {
    it('should throw error if fetching fails', async () => {
      ...
      await expect(() =>
        dappConnectorApiMaker({request: fakeFetchData}).getDApps({
    ...
    it('should throw error if response is invalid', async () => {
      ...
      await expect(() =>
        dappConnectorApiMaker({request: fakeFetchData as any}).getDApps({
          network: Chain.Network.Mainnet,
        }),
      ).rejects.toThrow()
    })
    ...
    it('should throw error if response is left', async () => {
      const fakeResult = {
        tag: 'left' as const,
        error: {status: 404, message: 'Not found', responseData: null},
      } as const
      const fakeFetchData = () => Promise.resolve(fakeResult)
      await expect(() =>
        dappConnectorApiMaker({request: fakeFetchData as any}).getDApps({
    ...
    it('should return data if response is valid', async () => {
      const fakeResult = {
        tag: 'right' as const,
        value: {data: await managerMock.getDAppList()},
      } as const
      const fakeFetchData = () => Promise.resolve(fakeResult)
      const result = await dappConnectorApiMaker({
        request: fakeFetchData as any,
      }).getDApps({
        network: Chain.Network.Mainnet,
      })
