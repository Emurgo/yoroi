...
export const cardNFT = (nftName: string) =>
 element(by.id(`card_nft_${nftName}`))
 ...
 while (i < max) {
  try {
   ...
   const nftAttributes = await element(by.id(/^card_nft_[a-zA-Z0-9_$. ]+$/))
    ...
    .getAttributes()
   if (
    'label' in nftAttributes &&
    nftAttributes.label?.toLowerCase().includes(nftLabel.toLowerCase())
   )
    return true
   i++
  } catch (e) {
...
export const getElementAttributes = async (
 ele: Detox.IndexableNativeElement,
) => {
...
export const verifyNftCount = async (
 ele: Detox.IndexableNativeElement,
 nftCountExp: number,
) => {
